...
char seg_data[] = {0x3f, 0x06, 0x5b, 0x4f, 0x66, 0x6d, 0x7d, 0x07, 0x7f, 0x6f};  // 0~9 + 0x80 就能显示冒号
// 开始一次数据传输
void tm1637_start() {
    // 拉高CLK，准备开始制造DIO下降沿
...
void write_bit(char bit) {
    digitalWrite(CLK_PIN, 0);  // 将时钟线（CLK_PIN）设置为低电平，准备开始一次数据传输
    ...
    digitalWrite(CLK_PIN, 1);  // 将时钟线设置为高电平，结束一次数据传输。
}
// 写入1个字节的数据
void write_byte(char data) {
    char i = 0;
    for (i = 0; i < 8; i++)
        write_bit((data >> i) & 0x01);  // 提取出最后一位bit
    ...
    while (digitalRead(DIO_PIN) && timeout != 0) {  // 等待数据线变为低电平，表示芯片已经接收完所有的数据，然后将数据线重新设置为输出模式。
        timeout--;
        delayMicroseconds(1);
    }
...
void time_display(int h_shi, int h_ge, int m_shi, int m_ge) {
    ...
    write_data(BCD_ADDR_2, seg_data[h_ge]);
